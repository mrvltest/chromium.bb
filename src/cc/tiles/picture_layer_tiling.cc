...
scoped_ptr<PictureLayerTiling> PictureLayerTiling::Create(
  ...
  return make_scoped_ptr(new PictureLayerTiling(
      tree, contents_transform, raster_source, client, tiling_interest_area_padding,
...
// static
gfx::Vector2dF PictureLayerTiling::CalculateSoonBorderDistance(
    const gfx::Rect& visible_rect_in_content_space,
    const gfx::Scaling2d& content_to_screen_scale) {
  return gfx::Vector2dF(
    std::min(
      kMaxSoonBorderDistanceInScreenPixels / content_to_screen_scale.x(),
      visible_rect_in_content_space.width() * kSoonBorderDistanceViewportPercentage),
    std::min(
      kMaxSoonBorderDistanceInScreenPixels / content_to_screen_scale.y(),
      visible_rect_in_content_space.height() * kSoonBorderDistanceViewportPercentage));
}
...
bool PictureLayerTiling::ComputeTilePriorityRects(
  ...
  // We keep things as floats in here.
  {
    gfx::RectF visible_rectf_in_content_space =        
        contents_transform_.MapRect(gfx::RectF(viewport_in_layer_space));
    ...
    const gfx::Vector2dF
        pad_in_content_space(
          tiling_interest_area_padding_ / content_to_screen_scale.x(),
          tiling_interest_area_padding_ / content_to_screen_scale.y());
    ...
    if (!eventually_rectf.IsEmpty()) {
      eventually_rectf.Inset(-pad_in_content_space.x(), -pad_in_content_space.y());
  ...
  gfx::Vector2dF border = CalculateSoonBorderDistance(visible_rect_in_content_space,
                                                      content_to_screen_scale);
  soon_border_rect.Inset(-border.x(), -border.y(), -border.x(), -border.y());
